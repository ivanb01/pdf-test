...
export default function GmailAILogs() {
  ...
  return (
    <div className="container mx-auto p-6">
      ...
      <p className="mb-4">Early preview: Gmail AI Logs</p>

          
      <div>
        <h2 className="text-xl font-bold mb-2">Gmail AI Logs:</h2>
        {logs.length > 0 ? (
          <table className="table-auto w-full">
            <thead>
              <tr>
                
                <th className="px-4 py-2">Email from:</th>
            ...
            <tbody>
              {logs.map((log, index) => (
                <React.Fragment key={index}>
                  <tr
                    className={logs.result_contact_stored_in_db ? 'bg-green2' : 'bg-gray2'}
                    onClick={() => toggleRow(index)}
                  >
                    
                    <td className="px-4 py-2">{log.email_from}</td>
                    <td className="px-4 py-2">
                      {(log.ai_contact_category_1 || log.ai_contact_category_2) ? (
                        `${log.ai_contact_category_1 || ''} ${log.ai_contact_category_2 || ''}`
                      ) : (
                        log.result_contact_category_id === 1 ? 'Uncategorized' : log.result_contact_category_id
                      )}
                    </td>

                    <td className="px-4 py-2">
                      {log.ai_email_summary ? (
                        `${log.ai_email_summary} - `
                      ) : (
                        'No AI summary - '
                      )}
