          ...
          <div className="flex flex-col justify-center items-center p-[26px] pt-[22px] border-dashed border-2 leading-3 font-normal text-xs text-gray5 rounded-md">
            <div className="w-full">
              ...
              <FileInputUpload.Uploaded>
                {(context) => {
                  const { postFileData } = context;
                  const { totalLoading, postFileMutationData, uploadedFileData } = postFileData;
                  const { isError: errorOnUplod, uploadProgress } = postFileMutationData;

                  return (
                    <div className="flex gap-2">
                      <div className="flex flex-col w-full gap-1.5">
                        <span className="text-xs font-medium text-gray7 leading-4">
                          {uploadedFileData?.name_with_format}
                        </span>
                        <ProgressBar
                          currentProgress={uploadProgress}
                          error={errorOnUplod}
