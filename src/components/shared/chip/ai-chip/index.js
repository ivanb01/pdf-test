...
const AIChip = ({ reviewed, className }) => {
  return (<>
    <MyTooltip side={'right'} align='center'
      ...
      <div style={{ width: '230px' }} className={'flex flex-col gap-4'}>
        {!reviewed ? <ChipText title={'New Contact'}
                              reviewed
                              description={' This contact is new, and you haven’t review it yet. For review, click on card to see more details.'} />
          : <ChipText title={'Reviewed Contact'}
                      reviewed={false}
...
const ChipText = ({ title, description, reviewed }) => {
  return <div>
    <div className={'flex gap-1.5 text-white items-center'}>
      ...
      <h6 className={'text-xs leading-4 font-semibold m-0'}>
        {title}
      </h6>
    </div>
