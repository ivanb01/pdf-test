      ...
      <tbody>
        {data.map((template) => (
          <tr
            ...
            <td className="px-3 py-2 text-left text-sm leading-5 flex flex-col h-[76px] gap-1 justify-center">
              <p className={'text-sm leading-5 font-medium'}>Subject:</p>
              <p className={'text-gray4 text-sm leading-8 font-normal '}>
                {isEmail
                  ? template.body_text.replace(/<\/?[^>]+(>|$)|&[a-zA-Z0-9#]+;/g, '')
                  : template.message.replace(/<\/?[^>]+(>|$)|&[a-zA-Z0-9#]+;/g, '')}
              </p>
            </td>
            <td className="px-3 py-2 text-gray7 text-center text-sm leading-5">
