  ...
  return (
    <div className={'p-6 flex-1 flex flex-col gap-[38px] bg-white border border-[#EFF4F5] rounded-lg mb-[48px]'}>
      ...
      <div>
        ...
        <div className={'flex justify-between align-center mt-[30px] flex-wrap'}>
          <div className={'flex gap-2 justify-center items-center'}>
            <div className={'h-[48px] w-[48px] flex items-center justify-center bg-[#F3F4F6] rounded-full'}>
              <LocalPhoneOutlinedIcon />
            </div>
            <div className={'flex flex-col gap-2'}>
              <p className={'text-sm font-medium text-gray6'}>Phone Support </p>
              <p className={'text-md font-semibold text-gray6'}>(646) 389-4548</p>
            </div>
          </div>
          <div className={'flex gap-2 justify-center items-center'}>
            <div className={'h-[48px] w-[48px] flex items-center justify-center bg-[#F3F4F6] rounded-full'}>
            ...
            <div className={'flex flex-col gap-2'}>
              <p className={'text-sm font-medium text-gray6'}>Email Support</p>
              <p className={'text-md font-semibold text-gray6'}>az@opgny.com</p>
            </div>
          </div>
        </div>
      </div>
    ...
    <div className='flex-1 p-6 flex flex-col gap-[38px] bg-white border border-[#EFF4F5] rounded-lg mb-[48px]'>
      ...
      <div className='flex justify-between flex-col flex-1'>
        <div className='flex gap-[10px] '>
          ...
          {!needToCommunicateBox ? <div className={'text-[#101828]'}>
          ...
          </div> : <div className={'text-[#101828]'}>
            <p className={'text-md font-bold'}>Need To Contact</p>
            <p className={'text-sm font-normal max-w-[292px]'}>You need to regularly contact clients to maintain good
              communication.</p>
          </div>}
        ...
        <div className='flex justify-between items-center ml-7'>
          ...
          <div className='h-[48px] w-[48px] rounded-full bg-gray1 flex items-center justify-center' role={'button'}
               onClick={() => {
...
const SuccessContactsContainers = ({ needToCommunicateBox, needToReview, needToContactCount }) => {
  const router = useRouter();
  return <div className={'flex-1 p-6 flex flex-col gap-[38px] bg-white border border-[#EFF4F5] rounded-lg mb-[48px]'}>
        ...
        <div>
          <svg width='60' height='60' viewBox='0 0 60 60' fill='none' xmlns='http://www.w3.org/2000/svg'>
            <path
              ...
              stroke='#059669' strokeWidth='1.66667' strokeLinecap='round' strokeLinejoin='round' />
          </svg>
        </div>
        <p
          className={'font-bold text-md'}>{!needToCommunicateBox ? 'No Smart Sync contacts to review!' : 'Communication health is great! No clients need to be contacted!'}
        < /p>
      </div>
      <div className={'flex justify-between items-center ml-7'}>
        <p className='font-semibold text-[30px]'>{!needToCommunicateBox ? needToReview : needToContactCount}</p>
...
const AgentDashboardLayout = ({ success, needToContactCount, needToReview }) => {
  ...
  return (
    <div className={'bg-[#F1F6F8] mt-7 h-full mx-4 rounded-t-[22px] flex flex-col gap-8 pt-8'}>
      <div className={'mx-9'}>
        <p className={'font-semibold text-3xl'}>Welcome {userInfo?.first_name ?? ''}!</p>
        <p className={'text-sm font-medium text-[#475467] mt-[10px]'}>
          We’re here to help you close more deals, stay on top of your clients and relationships & make sure you
          love
          where you work. Any feedback on how we can improve is encouraged and welcome! For software support click
          the
      ...
      {showInfo && <div
        className={
          'rounded-xl border border-[#FEF3C7] bg-[#FFFBEB] mx-6 p-4 flex gap-3 items-start justify-center text-black'
        }
      >
        <InfoOutlinedIcon className={'h-5 w-5'} />
        <div className={'flex-1'}>
          <p className={'mb-1 text-medium text-sm'}>Update profile</p>
        ...
        <div className={'p-2 bg-[#FEF3C7] flex items-center justify-center rounded-lg'} role={'button'}
             onClick={() => setShowInfo(false)}>
          <ClearOutlinedIcon className={'h-4 w-4 text-[#D97706]'} />
        </div>
      </div>}
      <hr className={'bg-[#D9E6EC] w-full mx-6'} />
      <div className={'flex mx-[22px] gap-[22px] '}>
        <NeedHelpContainer />
        ...
        {success && needToContactCount == 0 ?
          <SuccessContactsContainers needToCommunicateBox needToReview={needToReview}
                                     needToContactCount={needToContactCount} /> :
          <ContactsContainers needToCommunicateBox needToReview={needToReview}
                              needToContactCount={needToContactCount} />}
      </div>
    </div>);
