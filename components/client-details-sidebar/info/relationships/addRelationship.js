...
const AddRelationshipModal = ({handleClose, contactId, handleFetchRelationships}) => {
    ...
    const fetchContacts = async ()=> {
        try {
            const {data} = await contactServices.getContactsSearch({search_term: searchKey, exclude_contact_id: contactId});  
            console.log('contcts', data)        
            const filterData = data.map(item=>({id:item.id, name: `${item.first_name} ${item.last_name}`,email: item.email}))
    ...
    const handleChooseRelationshipType = (relationshipType) => {
        ...
        // )
        setRelationshipToAdd({...relationshipToAdd, relationship_type: relationshipType.name});
    }
    ...
    const addRelationship = async () => {
        try {
            ...
            const { data } = await contactServices.addContactRelationship(contactId, relationship);
        } catch (error) {
            console.log(error)
        }
    }
                            ...
                            <div className={`z-20 shadow rounded overflow-hidden absolute top-16 w-[100%] bg-white h-40`}>
                                <SimpleBar style={{ maxHeight: '100%' }}>
                                {
                                    contactsSearched.map((contact)=>(
                                        <div onClick = {()=>handleChooseContact(contact)} key={contact.id} className="flex flex-row p-3 hover:bg-lightBlue1 cursor-pointer group">
                                            <Avatar
                                                size="w-8 h-8"
                                                className="mr-4"
                            ...
                            <div className="flex flex-row p-3 bg-gray-50 group">
                                ...
                                <div className='flex flex-row justify-between w-[100%]'>
                                    ...
                                    <div className='flex flex-row items-center'> 
                                        ...
                                        ></Dropdown>
                                        <MinusCircleIcon onClick={removeRelationship} className="cursor-pointer text-gray3 hover:text-red4 m-2" height={20} />                                      
                                    </div>
